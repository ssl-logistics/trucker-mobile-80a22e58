...
const Map: React.FC<MapProps> = ({ latitude, longitude, markerLabel, showRoute = false }) => {
  ...
  // Get user location separately
  useEffect(() => {
    if (!showRoute || !navigator.geolocation) return;
  ...
  // Add routing only when map is ready and user location is available
  useEffect(() => {
    if (!mapReady || !showRoute || !userLocation || !map.current) return;
  ...
  }, [mapReady, showRoute, userLocation, latitude, longitude]);
