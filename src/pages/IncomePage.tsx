      ...
      <div className="px-4 pt-6">
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="space-y-4">
            {Object.keys(allGrouped).length === 0 ? <div className="text-center py-8 text-muted-foreground">ไม่มีข้อมูลรายได้</div> : Object.entries(allGrouped).map(([month, jobs]) => <div key={month}>
          ...
          <TabsContent value="paid" className="space-y-4">
            {Object.keys(paidGrouped).length === 0 ? <div className="text-center py-8 text-muted-foreground">ไม่มีงานที่ชำระแล้ว</div> : Object.entries(paidGrouped).map(([month, jobs]) => <div key={month}>
                  <div className="text-sm text-muted-foreground mb-2">{month}</div>
                  {jobs.map(income => <Card key={income.id} className="p-4 mb-3">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
          ...
          <TabsContent value="unpaid" className="space-y-4">
            {Object.keys(unpaidGrouped).length === 0 ? <div className="text-center py-8 text-muted-foreground">ไม่มีงานที่ยังไม่ชำระ</div> : Object.entries(unpaidGrouped).map(([month, jobs]) => <div key={month}>
                  <div className="text-sm text-muted-foreground mb-2">{month}</div>
                  {jobs.map(income => <Card key={income.id} className="p-4 mb-3">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
