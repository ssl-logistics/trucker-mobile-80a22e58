        ...
        <Tabs defaultValue="all" className="w-full">
          ...
          <TabsContent value="all" className="space-y-4">
            {Object.keys(allGrouped).length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">ไม่มีข้อมูลรายได้</div>
            ) : (
                  ...
                  {jobs.map((income) => (
                    <Card key={income.id} className="p-4 mb-3">
                      <div className="flex items-start justify-between mb-3">
                        ...
                        <div
                          className={`flex items-center gap-2 font-semibold ${income.status === "paid" ? "text-green-600" : "text-muted-foreground"}`}
                        >
                          {income.status === "paid" ? (
          ...
          <TabsContent value="paid" className="space-y-4">
            {Object.keys(paidGrouped).length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">ไม่มีงานที่ชำระแล้ว</div>
            ) : (
          ...
          <TabsContent value="unpaid" className="space-y-4">
            {Object.keys(unpaidGrouped).length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">ไม่มีงานที่ยังไม่ชำระ</div>
            ) : (
